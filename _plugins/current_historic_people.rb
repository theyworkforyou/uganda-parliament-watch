  ...
  class CurrentHistoricPeople < Generator
    ...
    def generate(site)
      ...
      site.collections['people'].docs.each do |person|
        if person.data['memberships'].any? { |m| m['legislative_period_id'] == most_recent_term['id'] }
          person.data['layout'] = CURRENT_MP_LAYOUT
          person.data['current_membership'] = person.data['memberships'].find { |m| m['legislative_period_id'] == most_recent_term['id'] }
        else
